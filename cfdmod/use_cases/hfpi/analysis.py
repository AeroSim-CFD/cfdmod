...
def plot_global_stats_results(
    ...
    **plot_kwargs,
    ...
    if(plot_peaks):
        ax.plot(
            df["direction"], df[f"min_{d}"] * unit_conversion, "v:", label=f"Min ({txt_lg})", **kwargs
        )
        ax.plot(
            df["direction"], df[f"max_{d}"] * unit_conversion, "^:", label=f"Max ({txt_lg})", **kwargs
        )
    if plot_mean:
        ax.plot(df["direction"], df[f"mean_{d}"] * unit_conversion, "-", label="Mean", **kwargs)


def plot_global_stats_per_direction(stats: dict[str, pd.DataFrame], unit_conversion: float = 1 / 1e6, unit_name: str = "MN"):
    """"""
