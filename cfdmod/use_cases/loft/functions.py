...
def find_borders(triangles_vertices: np.ndarray) -> np.ndarray:
    ...
    for t_idx, tri in enumerate(triangles_vertices):
        triangles_edges[t_idx] = [tuple(sorted([tri[id_0], tri[id_1]])) for id_0, id_1 in [(0,1),(0,2),(1,2)]]
...
def remove_edges_of_internal_holes(vertices: np.ndarray, edges: np.ndarray) -> np.ndarray:
    ...
    Returns:
        edges. Same format, but less elements.
    """
    ...
    while len(vertices_to_analyze)>0:
        ...
        while len(edges_by_vertex[current_vertice])>0:
            ...
            next_vertice = current_edge[0] if current_edge[1]==current_vertice else current_edge[1]
...
def remove_edges_too_aligned_with_projection_direction(
    ...
    edge_directions[:,2] = 0
    angles_between_edges_and_projection = get_angle_between(ref_vec=edge_directions, target_vec=projection_diretion)
    mask_not_too_aligned = (angle_tolerance < angles_between_edges_and_projection) & (angles_between_edges_and_projection < 180-angle_tolerance)
...
def remove_edges_oposite_to_loft_direction(
    ...
    angles_0 = get_angle_between(
        ref_vec=vert_dir_from_center_0,
        target_vec=projection_diretion
    )
    angles_1 = get_angle_between(
        ref_vec=vert_dir_from_center_1,
        target_vec=projection_diretion
    )
...
def generate_loft_triangles(
    ...
    triangles_1 = np.stack([edge_verts[0],edge_verts_projection[1],edge_verts_projection[0]], axis=1)
