...
def directional_weibull_fit(
    data: pd.DataFrame, wind_direction_cuts: list[tuple[float, float]]
    ...
    for d_0, d_1 in wind_direction_cuts:
        if d_0 < d_1:
            dir_selection = (
                (data["wind_direction"] >= d_0) & (data["wind_direction"] < d_1) & valid_selection
            )
        else:
            dir_selection = (data["wind_direction"] >= d_0) | (
                data["wind_direction"] < d_1
            ) & valid_selection
        ...
        results[(round(d_0, 2), round(d_1, 2))] = (
            (incidence_probability, shape, scale),
            dir_selection.sum(),
        )
    return results
...
def weibull_shape_from_mean_and_std(mean, std):
    def f(k):
        return gamma(1 + 2.0 / k) / gamma(1 + 1.0 / k) ** 2 - 1.0 - (std / mean) ** 2
...
def get_weibull_probability_between_velocities(shape, scale, v_low, v_high) -> float:
    return weibull_min.cdf(v_high, c=shape, scale=scale) - weibull_min.cdf(
        v_low, c=shape, scale=scale
    )
