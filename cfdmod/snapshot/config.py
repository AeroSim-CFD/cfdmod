    ...
    points: Annotated[
        ...
        Field(
            ...
            description=(
                ...
                "During processing, points are transformed the same way as the mesh "
                "(transformation rotate/translate, then re-centering to z=0), so they stay fixed "
                "to the body across its transformation. z_offset is not applied when points is set."
            ),
        ),
    ]
    ...
    def at_least_one_valid_option_was_chosen(cls, data) -> ValueTagsConfig:
        ...
        if has_points:
            ...
            if not (has_spacing and has_padding):
                raise ValueError(
                    "Floating position set for spacing or padding, but one is empty. "
