...
def directional_gumbel_fit(
    data: pd.DataFrame, wind_direction_cuts: np.ndarray, events_per_year: int = 4
) -> dict[tuple[float, float], tuple[float, float, list[float]]]:
    ...
    for i in range(len(wind_direction_cuts)):
        ...
        if dir_selection.sum() == 0:
            continue
        results[(int(d_0), int(d_1))] = fit_gumbel(
            data[dir_selection], events_per_year=events_per_year
        )
    return results


def fit_gumbel_BR_MIS(
    data: pd.DataFrame, events_per_year: int = 4, reduced_variate_cut_point: float = -1
    ...
    """
    reduced_variates, selected_peaks = get_storm_peaks(
        data, events_per_year, reduced_variate_cut_point
    )
    # reduced_variates = get_reduced_variate(selected_peaks, reescale_multiple=events_per_year)
    reduced_variates = get_reduced_variate(
        selected_peaks, reescale_multiple=1
    )  # weird behavior with events>1
    a, U, _, *_ = scipy.stats.linregress(reduced_variates, selected_peaks)
    # return U+np.log(events_per_year)*a, a, selected_peaks
    return U + np.log(events_per_year) * a, a, selected_peaks


def fit_gumbel_MLE_MIS(
    data: pd.DataFrame, events_per_year: int = 4, reduced_variate_cut_point: float = -1
) -> tuple[float, float, list[float]]:
    """Fit Gumbel max values using raw data and the specifications for it
...
def fit_gumbel(data: pd.DataFrame, events_per_year: int = 4) -> tuple[float, float, list[float]]:
    ...
    """
    return fit_gumbel_BR_MIS(
        data=data, events_per_year=events_per_year, reduced_variate_cut_point=-1
    )


def get_storm_peaks(
    data: pd.DataFrame, events_per_year: int, reduced_variate_cut_point: float
) -> list[float]:
...
def remove_storm_from_series(
    data: pd.DataFrame, peak_value: float, correlation_hours: float = 4 * 24
):
...
def plot_gumbel_pdf(
    list_of_maxima: list, U: float, a: float, events_per_year: int = 4, bins="auto"
):
