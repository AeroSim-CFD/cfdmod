    ...
    def _check_consistency(self) -> "DataSource":
        ...
        for col in ("position", "area", "volume", "normal"):
            ...
            if arr is not None and arr.shape[0] != n:
                raise ValueError(
                    f"elements.{col} length {arr.shape[0]} does not match "
                    f"n_elements={n}"
                )
        for gname, grouping in self.groupings.items():
            if grouping.n_elements != n:
                raise ValueError(
                    f"grouping {gname!r} has {grouping.n_elements} entries; "
                    f"expected {n}"
                )
...
class ModesDataSource(DataSource):
    ...
    def _check_modes(self) -> "ModesDataSource":
        if self.topology is not None:
            raise ValueError(
                "ModesDataSource does not carry a topology; got "
                f"{self.topology.cell_type!r}"
            )
        return self
