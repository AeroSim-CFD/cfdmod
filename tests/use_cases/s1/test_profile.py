...
class TestS1Profile(unittest.TestCase):
    def test_vtm_profiles(self):
        ...
        self.assertEqual(s1_profile.pos.shape, s1_profile.values.shape)
        self.assertEqual(probe_profile.pos.shape, probe_profile.values.shape)
        self.assertEqual(pitot_profile.pos.shape, pitot_profile.values.shape)
    ...
    def test_csv_profiles(self):
        ...
        for file in [
            ...
            self.assertEqual(s1.pos.shape, s1.values.shape)
            self.assertEqual(probe.pos.shape, probe.values.shape)
        self.assertEqual(pitot.pos.shape, pitot.values.shape)
