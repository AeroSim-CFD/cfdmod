...
The **Pressure Coefficient**, $c_p$, is a dimensionless quantity that provides a **generalized representation** of the pressure distribution on a surface, or body, exposed to a fluid flow.
...
1. **Body pressure** (`kind: surface`): per-timestep pressure on every
   mesh triangle.
2. **Reference probe** (`kind: points`, optional): atmospheric reference
...
The [calculate_cp.ipynb](calculate_cp.ipynb) notebook walks through this
