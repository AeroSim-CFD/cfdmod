...
:::{note}
Check out the [concepts](../concepts.md) section for more information about **region and surface** definitions.
:::
...
1. `mesh_attach` -- pull per-triangle areas, normals and centroids from
   the `.lnas` (or `.h5`) mesh.
2. `zoning_grouping` -- partition the mesh into an
   `x_intervals` x `y_intervals` x `z_intervals` box grid;
   triangles are assigned to a zone by centroid.
3. `field_series_for_groups` with `agg: area_weighted_mean` -- collapse
...
The [calculate_Ce.ipynb](calculate_Ce.ipynb) notebook walks through this
...
:::{note}
The rule for determining the region_idx is based on the **region index and the surface name**.
