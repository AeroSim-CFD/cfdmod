...
In order to assess the effects of pressure in $p_1$ and $p_2$, data of a probe far away from the building must be obtained.
This data is a time series of the **static reference pressure**, which is used as $p_{\infty}$.
...
For example, the value for the speed of sound in LBM units is $c_s^2=\frac{1}{3}$, while in SI is 340 m/s.
:::

The pressure signals obtained with Nassu solver are exported using LBM density units such as $\rho$.
...
The speed of sound ($c_s$) is defined as $c_s^2=\frac{1}{3}$ for our LBM modeling.
To transform this pressure to an dimensionless form, it needs to be divided by a **dynamic pressure** $q$:
...
Coefficients <./coefficients/index.md>
```
