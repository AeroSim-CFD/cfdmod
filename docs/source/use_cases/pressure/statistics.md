...
For example, the minimum and maximum values of a generic signal ($x$) is:
...
For example, the mean value of a generic signal ($x$) is:

$$
\bar{x} = \mu = \frac{\int x(t) dt}{T} = \frac{1}{N} \sum{x_i}
$$

Where $t$ is temporal variable, and $T$ is the sample total time.
For discrete signal $N$ is the size of the sample.

As mentioned before, the sample can be subdivided in $n$ subsamples.
Thus to compose the global average ($\mu$) from each subdivision average, a weighted average must be performed based on each subdivision size $N_n$:
...
For example, the RMS value of a generic signal ($x$) is:
...
Where $\beta$ and $\mu$ are parameters of the fit. 
The value for reduced variable $y$ commonly used is 1.4, resulting in 78% of non-exceeding extreme values.
...
Using this method, the extreme value is calculated provided a peak factor $f$.
Peak value is then defined using the RMS $\tilde{x}$ and average values $\bar{x}$.
...
This correction is performed using the values for the statistical factors ($S_2$) from the {footcite:t}`nbr19886123` 6123.
