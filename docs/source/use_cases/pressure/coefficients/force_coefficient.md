...
The **Force Coefficient**, $C_f$, is a dimensionless parameter that provides a generalized representation of the **resultant forces** experienced by an object within a fluid flow.
It offers a means to evaluate the **cumulative effect** of pressure coefficients, $c_p$ across different regions of an object's surface and how these pressures translate into aerodynamic forces.
...
1. `mesh_attach` -- pull per-triangle areas, normals and centroids from
   the `.lnas` (or `.h5`) mesh.
2. `body_grouping` -- assign each triangle to a body. An empty surface
...
4. `field_series_for_groups` -- sum each direction over each body.
...
The [calculate_Cf.ipynb](calculate_Cf.ipynb) notebook walks through this
