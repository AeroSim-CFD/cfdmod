...
with the outward normal :math:`\hat{n}_t` and triangle area :math:`A_t`, and
...
- ``"peak-factor"`` -- Davenport :footcite:t:`davenport1964gust`:
  :math:`\hat{x} = \bar{x} + g\,\sigma_x`, with the **gust peak factor**

  .. math::
     g = \sqrt{2 \ln(\nu T)} + \frac{0.5772}{\sqrt{2 \ln(\nu T)}}

  where :math:`\nu` is the mean up-crossing rate (taken as the response
  frequency :math:`f_0`, in Hz) and :math:`T` the averaging duration
...
1. The per-floor load coefficients are dimensionalised into a floor-load
   ``PointsDataSource`` (per-floor :math:`f_x`, :math:`f_y`, :math:`m_z`) by
...
2. Each mode shape is **mass-normalized to unit generalized mass**,
   :math:`\sum_i M_i\,(\phi_{x,i}^2 + \phi_{y,i}^2 + (R_i\,\phi_{rz,i})^2) = 1`,
...
3. The floor loads are projected onto each mode to a generalized modal load,
   ...
   is integrated in time (RK45) with damping ratio :math:`\xi_j` and natural
   frequency :math:`\omega_j`, and the modal coordinates are recomposed into
   per-floor displacements (:math:`d_x`, :math:`d_y`) and torsional rotation
   (:math:`r_z`).
4. The response is reported as per-floor displacements plus
   **static-equivalent** floor forces / moments (:math:`f_{eq,x}`,
   :math:`f_{eq,y}`, :math:`m_{eq,z}`) that the structural engineer applies
...
SI :math:`m/s^2` (convert to milli-g with
:func:`~cfdmod.building.mps2_to_milli_g`).

**NBR 6123** (serviceability limit, sec. 9.6.2), a decreasing power law in the
fundamental sway frequency :math:`f_0` :footcite:t:`nbr19886123`:

.. math::
   a_{lim} = 0.01\, k_c\, f_0^{-0.445} \quad [m/s^2]

with :math:`k_c = 4.08` (residential) or :math:`6.12` (commercial / office),
valid over :math:`0.06 \le f_0 \le 1.00` Hz at a 1-year return period.

**Melbourne and Palmer (1992)** serviceable-acceleration curve, a function of
:math:`f_0` and the return period :math:`R` in years
...
with :math:`T = 600` s, valid over :math:`0.06 < f_0 < 1.0` Hz and
:math:`0.5 < R < 10` years.
...
- **Effective-load envelopes** -- per-floor :math:`F_x` / :math:`F_y` /
  :math:`M_z`, one column per wind direction, for ``min`` / ``max`` (the signed
